/* Slide : Problématique — generic content-grid entrance */
document.addEventListener('DOMContentLoaded', () => {

    const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

    /* ═══ Smooth background (shared pattern) ═══ */
    ...
    const orbColors = [
        { r: 0, g: 119, b: 204 }, { r: 99, g: 102, b: 241 },
        { r: 16, g: 185, b: 129 }, { r: 139, g: 92, b: 246 },
    ];
    for (let i = 0; i < 5; i++) {
    ...
    /* ═══ Entrance sequence (guarded — works whether or not each element exists) ═══ */
    ...
    setTransition(hub, 'opacity 0.8s cubic-bezier(0.22,1,0.36,1), transform 0.8s cubic-bezier(0.22,1,0.36,1)', 900);
    setTransition(platforms, 'opacity 0.6s ease, transform 0.6s ease', 1100);
    setTransition(modLabel, 'opacity 0.6s ease, transform 0.6s ease', 1200);
    modCards.forEach((card, i) => {
        setTransition(card, 'opacity 0.6s cubic-bezier(0.22,1,0.36,1), transform 0.6s cubic-bezier(0.22,1,0.36,1)', 1300 + i * 100);
    });
    setTransition(nav, 'opacity 0.8s ease, transform 0.8s ease', 2000);
});
